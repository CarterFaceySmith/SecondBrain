A `weak_ptr` is a type of [[Smart Pointers]] in [[C++]] that adds a level of indirection and safety to a raw [[Pointer]]. It is mainly used to break reference cycles in cases where two objects have [[C++ Shared Pointer]]s to each other, or when you need a non-owning reference to an object that is managed by a `shared_ptr`.

A `weak_ptr` does not increase the _ownership_ reference count of the object it points to, which is a key difference between `weak_ptr` and `shared_ptr`. 
